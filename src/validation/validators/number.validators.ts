...
export const between = (min: number, max: number, message?: string) =>
    z
        ...
        .min(min)
        .max(max)
        .describe(message || `Must be between ${min} and ${max}`)

export const positive = (message = 'Must be positive') => z.number().positive(message)

export const nonNegative = (message = 'Must be zero or greater') => z.number().nonnegative(message)

export const integer = (message = 'Must be a whole number') => z.number().int(message)
